...
A [crate](https://github.com/dtolnay/anyhow) for working with trait object error types. Extends the features of the standard library's `Error` trait. However, most of Anyhow's features have been added to std, so you might not need Anyhow any more. The `bail` and `ensure` macros are pretty useful though!
...
[Eyre](https://github.com/yaahc/eyre) is a fork of Anyhow with enhanced reporting facilities.

The following crates are just for reporting and work with other error libraries if required:

* [miette](https://github.com/zkat/miette) (unsure if still maintained).
* [ariadne](https://codeberg.org/zesterer/ariadne).
