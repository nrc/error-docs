...
Both variants take a single argument which can be any type. You'll usually see `Result<T, E>` where `T` is the type of value returned in `Ok` and `E` is the error type returned in `Err`. It's fairly common for modules to use a single error type and to define an alias like `pub type Result<T> = std::result::Result<T, MyError>;`. In that case, you'll see `Result<T>` as the type, where `MyError` is the implicit error type.

Creating a `Result` value is as simple as using the variant as a constructor. E.g., for a `Result<i32, i32>` (a `Result` with an `i32` payload and an `i32` error code), you would use `Ok(42)` and `Err(2)` to create `Result` values.

When receiving a `Result` value, you can address the ok and error cases by using a `match` expression, e.g.,

```rust
fn use_match_to_handle_result(r: Result<String, MyError>) {
    match r {
...
There are more ergonomic ways to handle `Result`s too. There is a cornucopia of combinator methods on `Result`. See the [docs](https://doc.rust-lang.org/stable/std/result/enum.Result.html) for details. There are way too many to cover here, but as an example, `map` takes a function and applies it to the payload of the result if the result is `Ok` and does nothing if it is `Err`, e.g.,

```rust
fn map_a_result(r: Result<i32, MyError>) -> Result<String, MyError> {
    r.map(|i| i.to_string())
}
```

TODO unwrap, expect, is_ok, is_err

### The `?` operator

Applying `?` to a result will either unwrap the payload, if the result is `Ok`, or immediately return the error if it is `Err`. E.g.,

```rust
fn use_q_mark(r: Result<i32, MyError>) -> Result<String, MyError> {
    let i = r?; // Unwraps Ok, returns an Err
    Ok(i.to_string())
}
```

The above code does the same as the previous example. In this case, the `map` code is more idiomatic, but if we had a lot of work to do with `i`, then using `?` might be better. Note that the type of `i` is `i32` in both examples.

The `?` operator can be used with other types too, notably `Option`. With nightly Rust, you can use it with your own types, see the discussion of the `Try` trait, below.
...
```rust
fn use_q_mark_with_from(r: Result<i32, Error1>) -> Result<String, Error2> {
    let i = r?;
    // ...
}
```
...
`Option` is similar to `Result` in that it is a very common, two-variant enum type with a positive variant `Some` (compare to `Result::Ok`). The difference is that `Option::None` (compared with `Result::Err`) does not carry a payload. It is, therefore, analogous to `Result<T, ()>` and there are many methods for converting between `Option` and `Result`. `match`, `?`, `if let`, etc. work with `Option` similarly to `Result`.
...
The error trait, [`std::error::Error`](https://doc.rust-lang.org/nightly/std/error/trait.Error.html), is a trait for error types. There is no hard requirement for errors to implement it; you can use a type in a `Result` and with `?` without implementing `Error`. It has some useful functionality, and it means you can use dynamic error handling using `dyn Error` types. Generally you *should* implement `Error` for your error types (there are no required methods, so doing so is easy); most error libraries will do this for you.

### Provided functionality

`Display` and `Debug` are super-traits of `Error` which means that you can always convert an error into a string. Most error libraries let you derive the `Display` impl using an attribute with a format string.
...
To make this work, you implement `Error` for your concrete error types. If you want to make use of downcasting (see below), errors must not contain any borrowed data (i.e., they have a `'static` bound). For ease of use, you'll probably want to provide a constructor which returns the abstract type (e.g., `Box<dyn Error>`) rather than the concrete type. Creating and propagating errors works the same way as using concrete error types.

Handling errors might be possible using the abstract type only (using the `Display` impl, `source` and `sources` methods, etc.), or you can downcast the trait object to a concrete type (using one of the `downcast` methods). Usually, there are many possibilities for the concrete type of the error, you can either try downcasting to each possible type (the methods return `Option` or `Result` which facilitates this) or use the `is` method to test for the concrete type. This technique is an alternative to destructuring concrete errors using `match`.


## Unstable stuff

The following features are unstable and only available using nightly Rust.
...
There is a deprecated macro, `try`, which does basically the same thing as `?`. It only works with `Result` and in editions since 2018, you have to use `r#try` syntax to name it. It is deprecated and there is no reason to use it rather than `?`; you might come across it in very old code though.
